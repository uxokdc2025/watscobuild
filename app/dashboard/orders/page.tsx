  ...
  return (
    <DashboardShell title="Open Orders" description="Track orders, delivery progress, and recent purchasing activity.">
      <div className="space-y-4">
        <section aria-labelledby="order-search-heading" className="rounded-lg border bg-background p-5 shadow-sm sm:p-6">
          <h2 id="order-search-heading" className="sr-only">Find an order</h2>
          <form onSubmit={search} className="max-w-xl">
            <label htmlFor="open-order-search" className="text-sm font-medium">Search</label>
        ...
        <section aria-labelledby="open-orders-heading" className="rounded-lg border bg-background p-4 shadow-sm sm:p-6">
          ...
          <div className="flex items-center gap-3 rounded-md bg-cyan-500 px-4 py-4 text-sm font-semibold text-white"><Info aria-hidden="true" className="size-4 shrink-0" /> {visibleOrders.length} Orders Available</div>
          <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[680px] border-collapse text-sm"><thead><tr className="border-b text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground"><th className="px-2 py-3">Order</th><th className="px-2 py-3">Date</th><th className="px-2 py-3">Status</th><th className="px-2 py-3">Fulfillment</th><th className="px-2 py-3 text-right">Total</th></tr></thead><tbody>{visibleOrders.map(([number, date, total]) => <tr key={number} className="border-b last:border-0"><td className="px-2 py-4 font-semibold text-primary">{number}</td><td className="px-2 py-4 whitespace-nowrap">{date}</td><td className="px-2 py-4"><span className="rounded bg-muted px-2 py-1 text-xs font-semibold">open</span></td><td className="px-2 py-4"><span className="inline-flex items-center gap-2 rounded-md border px-3 py-1.5 text-xs font-semibold"><Truck aria-hidden="true" className="size-4" /> Standard</span></td><td className="px-2 py-4 text-right font-semibold whitespace-nowrap">{total}</td></tr>)}</tbody></table></div>
